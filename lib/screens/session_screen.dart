...
import 'package:yoga_session_app/widgets/custom_dialogues.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) async {
        if (!didPop) {
          final shouldExit = await CustomDialogues.showExitConfirmationDialog(
            context,
          );
          if (shouldExit) {
            final provider = Provider.of<YogaSessionProvider>(
              context,
              listen: false,
            );
        ...
        body: SafeArea(
          child: Consumer<YogaSessionProvider>(
            builder: (context, provider, _) {
              final sequence =
                  provider.session!.sequence[provider.sequenceIndex];
